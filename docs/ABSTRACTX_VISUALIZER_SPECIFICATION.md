...
# 3. Or launch standalone with synthetic telemetry simulation
python3 tools/visualizer/abstractx_studio.py --sim
```

### Why `imgui-bundle` is the Ideal UI Technology:
1. **Ultra-High-Speed ImPlot Engine**: Plots hundreds of thousands of 8 kHz IMU points and Gantt bars at 60–120 FPS with near-zero CPU and GPU overhead.
2. **Immediate Mode Architecture**: Instant data updates without complex HTML/DOM overhead or WebSockets server setup.
3. **Pure Python Simplicity**: Easily scriptable, cross-platform (Linux, Windows, macOS), and unpacks binary CTF / TLP packets natively via Python `struct`.
